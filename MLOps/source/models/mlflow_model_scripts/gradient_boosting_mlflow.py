...
Run example:
    python gradient_boosting_mlflow.py

Optional CSV usage:
    python gradient_boosting_mlflow.py --csv-path ../data/ml_dataset.csv
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run tuned Gradient Boosting experiment and register best model in MLflow.")
    parser.add_argument(
        "--csv-path",
        default=None,
    ...
    parser.add_argument(
        "--experiment-name",
        default="Sales Demand Forecasting - Gradient Boosting",
        help="MLflow experiment name.",
    )
    parser.add_argument(
        "--registered-model-name",
        default="sales_gradient_boosting",
...
def main():
    ...
    with mlflow.start_run(run_name="gradient_boosting_shap_reduced_95"):
        ...
        mlflow.log_param("param_grid_model__learning_rate", param_grid["model__learning_rate"])
        ...
        mlflow.set_tag(
            "Training Info",
            "SHAP-reduced Gradient Boosting for next-period net sales forecasting",
        )
        mlflow.set_tag("model_key", "gradient_boosting")
        ...
        mlflow.set_tag("artifact_path", "gradient_boosting_model")
