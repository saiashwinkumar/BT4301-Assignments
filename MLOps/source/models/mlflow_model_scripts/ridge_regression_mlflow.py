...
Run example:
    python ridge_regression_mlflow.py

Optional CSV usage:
    python ridge_regression_mlflow.py --csv-path ../data/ml_dataset.csv
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run tuned Ridge Regression experiment and register best model in MLflow.")
    parser.add_argument(
        "--csv-path",
        default=None,
    ...
    parser.add_argument(
        "--experiment-name",
        default="Sales Demand Forecasting - Ridge Regression",
        help="MLflow experiment name.",
    )
    parser.add_argument(
        "--registered-model-name",
        default="sales_ridge_regression",
...
def main():
    ...
    val_metrics = evaluate_model(best_pipeline, X_val, y_val)
    ...
    with mlflow.start_run(run_name="ridge_regression_grid_search"):
        ...
        mlflow.log_param("param_grid_model__alpha", ridge_param_grid["model__alpha"])
        mlflow.log_params(grid_search.best_params_)
        mlflow.log_metric("best_cv_rmse", float(-grid_search.best_score_))
        ...
        mlflow.set_tag(
            "Training Info",
            "Tuned Ridge Regression for next-period net sales forecasting",
        )
        ...
        print("Best CV RMSE:", float(-grid_search.best_score_))
